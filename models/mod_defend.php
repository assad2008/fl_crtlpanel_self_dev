...
class mod_defend
{
	...
	{
		// 是否允许代理访问
		if (mod_config::get_one_config('fl_proxy') == 0)
		{
		...
		//CC防护设置
		$yl_cc = mod_config::get_one_config('fl_cc');
		if(substr(PHP_OS, 0, 3) != 'WIN')
		{
			if($fp = @fopen('/proc/loadavg', 'rb')) {
				list($loadavg) = explode(' ', fread($fp, 6));
				fclose($fp);
				mod_config::get_one_config('fl_loadavg') && $loadavg > mod_config::get_one_config('fl_loadavg') && $yl_cc = 2;
			}
		}
		if(!empty($yl_cc))
		{
