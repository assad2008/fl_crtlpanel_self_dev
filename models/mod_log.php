...
class mod_log
{
		...
		public static function get_lwlogline($day = false)
		{
			if(!$day)
			{
				$timestamp = time();
				$ymd = date('Ymd',$timestamp);
				$logfile = BASE_PATH."./api/data/log/lwlog{$ymd}.php";
			}else
			{
				$logfile = BASE_PATH."./api/data/log/lwlog{$day}.php";
			}
			$line = countfileline($logfile);
			return $line;			
		}

		public static function log_lw($day = false)
		{
       ...
       if(file_exists($logfile))
       {
        ...
        if ($count%$lwperpage==0)
        {
            $numofpage=floor($count/$lwperpage);
        }
        else
        {
            $numofpage=floor($count/$lwperpage)+1;
				}
        if ($page>$numofpage)
        {
            $page=$numofpage;
        }
        $pagemin=min(($page-1)*$lwperpage , $count-1);
				$pagemax=min($pagemin+$lwperpage-1, $count-1);
				$start=($page-1)*$lwperpage;
				$num = 0;
				foreach($logfiledata AS $v)
				{
                          if($num >= $start && $num < $start+$lwperpage)
                            {
															$detail=explode(",",$v);
                                $adlogfor.="
<tr class=b align='center'>
	...
	<td class='smalltxt'>$detail[7]</td>
	<td class='smalltxt'>$detail[8]</td>
</tr>";
                            }
                            $num++;					
				}
			 }
			 	$basename = '?c=log&a=log_lw';
        $pages=self::numofpage($num,$page,$numofpage,$basename."&day=".$day);
    ...
    public static function log_admin($type='admin')
    {
        ...
        $basename="?c=log".$url_add;
				...
				{
					$adminlogfile =  BASE_PATH."./api/data/phplog/php_error.log";
				}
...
<tr class=b align='center'>
	...
	<td class='smalltxt'>$detail[6]</td>
                ...
                case 'apilog':
                    throw new Exception("API脚本日志暂无查询功能");
                    break;
                    ...
                    if($phplogfiledata)
                    {
                        ...
                        for($i=$pagemin; $i<=$pagemax; $i++)
                        {
                            ...
                            (preg_match("/^\[([0-9]+\-[A-za-z]+\-[0-9]+\s[0-9]+:[0-9]+:[0-9]+)\]\sPHP\s(Warning|Parse\serror|Fatal\serror)\:\s\s(.+)/", $tmp[0], $matches))
                            {
                                ...
                                $winddate = date('Y-m-d H:i:s',strtotime($matches[1]));
...
<tr class=b align='center'>
	...
	<td class='smalltxt'>$detail[6]</td>
       ...
       return array($adlogfor,$pages);
		...
		public static function get_oplog($start,$keyword = false,$cols = false,$page = 40)
		{
			$data = array();
			$condition = '';
			if($keyword && $cols)
			...
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM pm_oplog $condition";//debug($sql);
