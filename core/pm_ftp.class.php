<?php
/**
* @file pm_ftp.class.php
* @synopsis  FTP功能
* @author Yee, <assad2008@sina.com>
* @version 1.0
* @date 2012-05-02 14:44:47
 */
...
!defined('PATH_ADMIN') && exit('Forbidden');
	...
	class FTP 
	{
		...
		function cd($path)   //进入目录
		{
			return $this->cwd($path);
		}

		function chdir($path)   //改变目录
		{
			return $this->cwd($path);
		}

		function chmod($mode,$filename)  //改变权限
		{
		...
		function exec($cmd)   //执行命令
		{
			return $this->site("EXEC $cmd");
		}

		function fget($fp,$remote,$mode = FTP_BINARY,$resumepos = 0)  //下载FTP上的文件 默认为二进制 存为资源
		{
			$this->_resetError();
		...
		function fput($remote,$resource,$mode = FTP_BINARY,$startpos = 0)  //复制一个打开文件到FTP
		{
			$this->_resetError();
		...
		function get_option($option)  //检查当前FTP运作
		{
			$this->_resetError();
		...
		function get($locale,$remote,$mode = FTP_BINARY,$resumepos = 0)  //下载FTP上的文件
		{
		...
		function nlist($remote_filespec = "")  //遍历目录文件
		{
			$this->_resetError();
		...
		function pasv($pasv) //启动passive模式
		{
			if (!$pasv) 
			{
		...
		function put($remote,$local,$mode = FTP_BINARY,$startpos = 0)   //上载本地文件到FTP
		{
		...
		function rawlist($remote_filespec = "")  //返回给定目录的文件详细信息
		{
			$this->_resetError();
		...
		function ls($remote_filespec = "")   ///返回解析后的目录
		{
		...
		function ssl_connect()  //打开安全ssl连接
		{
			$this->_resetError();
		...
		function systype()   //返回系统的远程FTP服务器的类型标识符
		{
			$this->_resetError();
